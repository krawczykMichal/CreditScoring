...
proc sql;
    select count(*)
    from out.abt_scored_final
    where active_customer_flag = 0;
quit;

proc contents data=out.abt_app varnum;
quit;

/* ==========================================================================
   ANALIZA FINANSOWA PRZYCZYN ODRZUCENIA (REJECTION IMPACT ANALYSIS)
   ========================================================================== */
options mprint symbolgen;

/* Upewniamy się, że mamy zdefiniowane parametry z poprzedniego kroku */
%let pd_css_cutoff  = 0.3788;
%let pd_ins_hard    = 0.3965;
%let pd_ins_soft    = 0.15;
%let pr_req         = 0.01;
...
data out.rejection_impact;
    set out.abt_scored_final;
    where '197501'<=period<='198712';
    
    /* 1. REKALKULACJA "TEORETYCZNEGO" ZYSKU (Gdybyśmy przyjęli wszystkich) */
    /* Musimy to policzyć od nowa, bo w symulacji wyzerowaliśmy zysk dla odrzuconych */
    ...
    /* Obsługa braków default */
    if default12 in (0,.i,.d) then default12=0;
    if default_cross12 in (0,.i,.d) then default_cross12=0;

    /* Finanse */
    if product='ins' then do; lgd=&lgd_ins; apr=&apr_ins/12; end;
    else if product='css' then do; lgd=&lgd_css; apr=&apr_css/12; end;
    ...
    Theoretical_Profit = Income - EL;

    /* Profit Cross-Sell */
    /* (Dla uproszczenia analizy skupiamy się na głównym produkcie, 
       ale dodajemy cross jeśli był w logice odrzucenia) */
    lgd_cross = &lgd_css; apr_cross = &apr_css/12;
    EL_cross = 0;
    if default_cross12=1 then EL_cross = cross_app_loan_amount * lgd_cross;
    ...
    Theoretical_Profit_Total = Theoretical_Profit + (Income_cross - EL_cross);


    /* 2. PRZYPISANIE PRZYCZYNY ODRZUCENIA (wg Nowej Logiki) */
    length sim_rejection_reason $50;
    sim_rejection_reason = 'ACCEPTED'; /* Domyślnie */

    /* Logika CSS */
    if product='css' then do;
        if pd_css > &pd_css_cutoff then sim_rejection_reason = 'Risk: CSS Cut-off (High PD)';
    end;

    /* Logika INS */
    if product='ins' then do;
        if pd_ins > &pd_ins_hard then sim_rejection_reason = 'Risk: INS Hard Cut-off (High PD)';
        else if (pd_ins > &pd_ins_soft) and (pr < &pr_req or pd_cross_css > &pd_css_cutoff) 
            then sim_rejection_reason = 'Risk: INS Cross-Sell Logic';
            
        /* Dodatkowo: Sprawdzenie reguł biznesowych (jeśli były w pierwotnym zbiorze) */
        /* Zakładamy, że powyższe reguły ryzyka są nadrzędne w tej symulacji */
    end;

run;

/* 3. RAPORTOWANIE SKUTKÓW FINANSOWYCH */
title "Analiza Finansowa Odrzuconych Wniosków (Ile zyskałeś lub straciłeś na decyzji)";
title2 "Ujemna kwota = Uniknięta strata (Dobrze) | Dodatnia kwota = Utracony zysk (Źle)";

proc tabulate data=out.rejection_impact;
    class product sim_rejection_reason;
    var Theoretical_Profit_Total;
    
    /* Analizujemy tylko odrzuconych */
    where sim_rejection_reason ne 'ACCEPTED';
    
    table product * sim_rejection_reason,
          N='Liczba Odrzuconych'*f=comma12. 
          Theoretical_Profit_Total * Sum='Wynik Finansowy Odrzuconych (PLN)'*f=comma20.0
          / box='Przyczyna Odrzucenia';
run;
title;
title2;